...
#import "UIImage+InvestigatorGal.h"
...
@implementation UIImage (InvestigatorGal)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(loops:));
...
+ (UIImage *)loops:(NSString *)name {
    ...
    UIImage *image = [[ApproximatelyFeather calendarManager] loops:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self loops:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
