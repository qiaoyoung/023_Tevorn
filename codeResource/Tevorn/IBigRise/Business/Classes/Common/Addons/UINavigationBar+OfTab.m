...
#import "UINavigationBar+OfTab.h"
...
@implementation UINavigationBar (OfTab)
...
- (void)eachSimultaneously{
    //: [self swizzling_layoutSubviews];
    [self eachSimultaneously];
    ...
    subview.shadowinessInsert = navigationBtnMargin;
...
- (void)signNose{
    ...
    CGPoint centerPonit = CGPointMake(self.property * .5f, self.acceptParent *.5f);
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(eachSimultaneously));
    //: });
    });
}
