...
@protocol AttenderDelegate <NSObject>
...
- (void)exasperate:(BeyondView *)menuView resultIndex:(CarteDuJourViewCell *)menuCell shelter:(NSInteger)index;
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage FilmBasedModel instances
...
@property (nonatomic, weak) id<AttenderDelegate> sweepResignsed;
...
@property (nonatomic, assign) double laddieTotal;
...
@property (nonatomic, assign) FFDropDownMenuViewAnimateType regularRest;
...
@property (nonatomic, assign) CGFloat access;
...
@property (nonatomic, strong) UIColor *triangleMulti;
...
@property (nonatomic, assign) CGFloat prompt;

@property (nonatomic, assign) NSInteger handleMagnitude;
...
@property (nonatomic, strong) UIColor *golfBagMode;

@property (nonatomic, strong) NSMutableArray *angleUpperArray;
...
@property (nonatomic, assign) NSInteger bigness;
...
@property (nonatomic, assign) CGFloat stateCapitalWilling;
...
@property (nonatomic, assign) CGFloat resFloat;
@property (nonatomic, assign) CGFloat writing;

///==================================================================
///                    创建下拉菜单方式2
///   适用于自定义下拉菜单样式，单个属性进行赋值
///   >>步骤:
///   1、 [BeyondView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
@property (nonatomic, assign) UIColor *statuteTitleColor;
/** 2、cell的类名, 必须是FFDropDownMenuBasedCell的子类
 *               若用框架自带的cell,直接传@"BolusViewCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"BolusViewCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of CarteDuJourViewCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"BolusViewCell"
...
@property (nonatomic, assign) CGFloat trustBill;
...
@property (nonatomic, assign) CGSize conditionedEmotionalResponseFragment;

@property (nonatomic, assign) CGFloat leadingOutputFloat;
 ...
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage FilmBasedModel instances or the FilmBasedModel's subclass instances )
 */
//: @property (nonatomic, strong) NSArray *menuModelsArray;
@property (nonatomic, strong) NSArray *theFront;

@property (nonatomic, assign) double translateTotal;


@property (nonatomic, assign) NSInteger personCounterval;

@property (nonatomic, strong) NSMutableArray *whetstoneArray;
...
@property (nonatomic, assign) CGSize coverDistance;
/** 弹出菜单 */
//: - (void)showMenu;
- (void)exist;
...
@property (nonatomic, assign) CGFloat finishAlpha;
@property (nonatomic, strong) NSMutableArray *aptArray;
@property (nonatomic, assign) double readingCount;
...
@property (nonatomic, assign) CGFloat longDuration;
@property (nonatomic, assign) NSInteger listenerMagnitude;
...
@property (nonatomic, assign) CGFloat sequence;
...
@property (nonatomic, assign) CGFloat bound;
...
@property (nonatomic, assign) CGFloat alongside;
