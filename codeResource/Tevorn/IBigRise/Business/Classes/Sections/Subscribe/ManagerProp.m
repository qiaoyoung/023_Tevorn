...
@interface CarAlwaysData : NSObject

+ (instancetype)sharedInstance;

//: online_state
@property (nonatomic, copy) NSString *userBrowStr;

//: net_state
@property (nonatomic, copy) NSString *mainModelMsg;

@end

@implementation CarAlwaysData

//: net_state
- (NSString *)mainModelMsg {
    if (!_mainModelMsg) {
		NSArray<NSString *> *origin = @[@"9", @"62", @"10", @"132", @"86", @"80", @"126", @"149", @"101", @"71", @"172", @"163", @"178", @"157", @"177", @"178", @"159", @"178", @"163", @"31"];
		NSData *data = [CarAlwaysData CarAlwaysDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _mainModelMsg = [self StringFromCarAlwaysData:value];
    }
    return _mainModelMsg;
}

//: online_state
- (NSString *)userBrowStr {
    if (!_userBrowStr) {
		NSArray<NSString *> *origin = @[@"12", @"82", @"13", @"18", @"138", @"60", @"232", @"136", @"11", @"160", @"214", @"254", @"10", @"193", @"192", @"190", @"187", @"192", @"183", @"177", @"197", @"198", @"179", @"198", @"183", @"208"];
		NSData *data = [CarAlwaysData CarAlwaysDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _userBrowStr = [self StringFromCarAlwaysData:value];
    }
    return _userBrowStr;
}

- (Byte *)CarAlwaysDataToCache:(Byte *)data {
    int valueWind = data[0];
    Byte browsing = data[1];
    int ideaFlash = data[2];
    for (int i = ideaFlash; i < ideaFlash + valueWind; i++) {
        int value = data[i] - browsing;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[ideaFlash + valueWind] = 0;
    return data + ideaFlash;
}

- (NSString *)StringFromCarAlwaysData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self CarAlwaysDataToCache:data]];
}

+ (NSData *)CarAlwaysDataToData:(NSArray<NSString *> *)value {
...
+ (instancetype)sharedInstance {
    static CarAlwaysData *instance = nil;
...
@property (nonatomic,strong) NSMutableSet *ringCollect;

//: @property (nonatomic,strong) NSMutableDictionary *events;
@property (nonatomic,strong) NSMutableDictionary *translate;

//: @property (nonatomic,strong) NSMutableSet *subscribeIds;
@property (nonatomic,strong) NSMutableSet *being;
@property (nonatomic,strong) NSMutableSet *vendorCollect;

//: @end
@end

//: @implementation NTESSubscribeManager
@implementation ManagerProp
...
- (NSDictionary<NIMSubscribeEvent *,NSString *> *)sparring:(NSInteger)type
{
    ...
    return [self.translate objectForKey:@(type)];
}
...
- (void)springGlobal
{
    ...
    event.type = NIMSubscribeSystemEventTypeOnline;
	[self setRingCollect:_vendorCollect];
    //: event.value = EnumCustomStateValueOnlineExt;
    event.value = EnumCustomStateValueOnlineExt;
    //: event.sendToOnlineUsersOnly = NO; 
    event.sendToOnlineUsersOnly = NO;
	[self setRingCollect:_vendorCollect]; //必须要让后登录的用户也能拿到    
    //: NSDictionary *ext = @{
    NSDictionary *ext = @{
                            //: @"net_state" : @([NTESDevice currentDevice].currentNetworkType),
                            [CarAlwaysData sharedInstance].mainModelMsg : @([SunrayDevice sum].acceptable),
                            //: @"online_state" : @(EnumOnlineStateNormal), //移动端永远在线
                            [CarAlwaysData sharedInstance].userBrowStr : @(EnumOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext listenerText]];
...
- (void)visualSystem
{
    ...
    [_being addObjectsFromArray:self.abstract.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_being addObjectsFromArray:self.columnIds.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self make:_being.allObjects];
}
...
- (void)student:(NSString *)userId
{
    ...
    if (![_being containsObject:userId])
    {
        //如果这个人没有订阅
        ...
        [[self flowCommand:self.vendorCollect] removeObject:userId];
    }
}
...
- (void)onRecvSubscribeEvents:(NSArray *)events
{
    ...
    for (NIMSubscribeEvent *event in events) {
        ...
        if ([self.being containsObject:event.from] || [[self flowCommand:self.vendorCollect] containsObject:event.from])
        {
            //: NSInteger type = event.type;
            NSInteger type = event.type;
            //: NSMutableDictionary *eventsDict = [self.events objectForKey:@(type)];
            NSMutableDictionary *eventsDict = [self.translate objectForKey:@(type)];
            //: if (!eventsDict) {
            if (!eventsDict) {
                //: eventsDict = [[NSMutableDictionary alloc] init];
                eventsDict = [[NSMutableDictionary alloc] init];
                //: [self.events setObject:eventsDict forKey:@(type)];
                [self.translate setObject:eventsDict forKey:@(type)];
            }
...
//: - (void)cleanCache
- (void)primaryNational
{
    //: [_subscribeIds removeAllObjects];
    [_being removeAllObjects];
    //: [_tempSubscribeIds removeAllObjects];
    [[self flowCommand:_vendorCollect] removeAllObjects];
    //: [_events removeAllObjects];
    [_translate removeAllObjects];
}
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        _being = [[NSMutableSet alloc] init];
        //: _tempSubscribeIds = [[NSMutableSet alloc] init];
        _vendorCollect = [[NSMutableSet alloc] init];
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (isMyFriend && ![self.being containsObject:user.userId])
    {
        //是好友却没订阅，订阅一下
        //: NIMSubscribeRequest *request = [self generateRequest];
        NIMSubscribeRequest *request = [self alongside];
...
+ (instancetype)calendarManager
{
    ...
    static ManagerProp *instance;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSubscribeManager alloc] init];
        instance = [[ManagerProp alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)disturbingTheoretical:(NSString *)userId
{
    ...
    [[self flowCommand:self.vendorCollect] addObject:userId];
    //: [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
    //: }];
    }];
           ...
           totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P) {
        ...
        [self.being addObject:recentSession.session.sessionId];
...
- (void)setRingCollect:(NSMutableSet *)ringCollect {
    //: OC_CUSTOM_PROPERTY_INJECT
    _ringCollect = ringCollect;
}



//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)make:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (NSMutableSet *)flowCommand:(NSMutableSet *)ringCollect {
    //: OC_CUSTOM_PROPERTY_INJECT
    _ringCollect = ringCollect;
    return ringCollect;
}
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.columnIds containsObject:recentSession.session.sessionId]) {
        //: [self.subscribeIds removeObject:recentSession.session.sessionId];
        [self.being removeObject:recentSession.session.sessionId];
    }
}
