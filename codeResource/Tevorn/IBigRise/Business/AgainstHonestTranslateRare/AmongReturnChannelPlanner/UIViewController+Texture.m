...
@interface RationalDecadeData : NSObject

@end

@implementation RationalDecadeData

//: back_arrow_bl
+ (NSString *)main_dreamId {
    /* static */ NSString *main_dreamId = nil;
    if (!main_dreamId) {
		NSArray<NSString *> *origin = @[@"13", @"4", @"133", @"145", @"108", @"98", @"95", @"119", @"111", @"114", @"114", @"97", @"95", @"107", @"99", @"97", @"98", @"159"];
		NSData *data = [RationalDecadeData RationalDecadeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        main_dreamId = [self StringFromRationalDecadeData:value];
    }
    return main_dreamId;
}

//: #222222
+ (NSString *)mainSadFormat {
    /* static */ NSString *mainSadFormat = nil;
    if (!mainSadFormat) {
		NSArray<NSString *> *origin = @[@"7", @"10", @"252", @"83", @"191", @"151", @"49", @"32", @"115", @"215", @"50", @"50", @"50", @"50", @"50", @"50", @"35", @"33"];
		NSData *data = [RationalDecadeData RationalDecadeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        mainSadFormat = [self StringFromRationalDecadeData:value];
    }
    return mainSadFormat;
}

+ (Byte *)RationalDecadeDataToCache:(Byte *)data {
    int dependenceMind = data[0];
    int meAbase = data[1];
    for (int i = 0; i < dependenceMind / 2; i++) {
        int begin = meAbase + i;
        int end = meAbase + dependenceMind - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[meAbase + dependenceMind] = 0;
    return data + meAbase;
}  

//: #875FFA
+ (NSString *)m_inspectionData {
    /* static */ NSString *m_inspectionData = nil;
    if (!m_inspectionData) {
		NSArray<NSString *> *origin = @[@"7", @"4", @"156", @"54", @"65", @"70", @"70", @"53", @"55", @"56", @"35", @"219"];
		NSData *data = [RationalDecadeData RationalDecadeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        m_inspectionData = [self StringFromRationalDecadeData:value];
    }
    return m_inspectionData;
}

+ (NSString *)StringFromRationalDecadeData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self RationalDecadeDataToCache:data]];
}

+ (NSData *)RationalDecadeDataToData:(NSArray<NSString *> *)value {
...
//: #612CF9
+ (NSString *)userEditPath {
    /* static */ NSString *userEditPath = nil;
    if (!userEditPath) {
		NSArray<NSString *> *origin = @[@"7", @"5", @"238", @"10", @"216", @"57", @"70", @"67", @"50", @"49", @"54", @"35", @"77"];
		NSData *data = [RationalDecadeData RationalDecadeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        userEditPath = [self StringFromRationalDecadeData:value];
    }
    return userEditPath;
}
...
#import "UIViewController+Texture.h"

//: @implementation UIViewController (KIAdditions)
@implementation UIViewController (Texture)
...
- (void)assistant{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[self streetwiseLight:[UIColor clearColor]] forBarMetrics:UIBarMetricsDefault];
...
- (void)onInstruction {
    ...
    [self customDisappear:@selector(globalOn)];
}

//: - (void)showCustomBackButton:(SEL)selector {
- (void)customDisappear:(SEL)selector {
    //: [self setNavLeftItem:selector image:[UIImage imageNamed:@"back_arrow_bl"] imageH:nil];
    [self sizeH:selector provider:[UIImage imageNamed:[RationalDecadeData main_dreamId]] origin:nil];

}

//: - (void)setNavLeftItem:(SEL)selector image:(UIImage *)image imageH:(UIImage *)imageH {
- (void)sizeH:(SEL)selector provider:(UIImage *)image origin:(UIImage *)imageH {
...
- (void)setTitle:(NSString *)title {
    ...
    [titleLabel setTextColor:[UIColor minimal:[RationalDecadeData mainSadFormat]]];
...
- (BOOL)manage:(UIView *)view theoryRole:(BOOL)hide{
    //: UIView *line = [self findHairlineImageViewUnder:view];
    UIView *line = [self fit:view];
...
- (void)underSheetH:(SEL)selector opera:(UIImage *)image underDay_strong:(UIImage *)imageH {
    //: UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    //: [backButton setImage:image forState:UIControlStateNormal];
    [backButton setImage:image forState:UIControlStateNormal];
...
- (void)flaunt:(NSString *)title render:(UIColor *)color{
    //: UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    //: if (!titleLabel) {
    if (!titleLabel) {
...
- (void)setCityWith:(UIColor *)color{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[SeeHelper sum:[UIColor minimal:[RationalDecadeData m_inspectionData]] tapType:[UIColor minimal:[RationalDecadeData userEditPath]] exception:SNLinearGradientDirectionLevel] forBarMetrics:UIBarMetricsDefault];
    //: self.navigationController.navigationBar.shadowImage = [UIImage new];
    self.navigationController.navigationBar.shadowImage = [UIImage new];
    //: self.navigationController.navigationBar.translucent = YES;
    self.navigationController.navigationBar.translucent = YES;
...
- (void)setIngredient:(UIColor *)color{
    //: UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    //: if (!titleLabel) {
    if (!titleLabel) {
...
- (void)setPlayerSecureReply:(CGFloat)alpha{
...
- (void)globalOn {
    ...
    [self exceptionController];
    //: [self popController];
    [self outfall];
}

//: - (UIImage *)imageWithColor:(UIColor *)color{
- (UIImage *)streetwiseLight:(UIColor *)color{
...
- (void)extent:(SEL)selector underperformer:(NSString *)title result:(UIColor *)color
{
    //: UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
...
- (void)league:(SEL)selector overIndependent:(NSString *)title perExtended:(UIColor *)color
{
    //: UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    //: [backButton setTitle:title forState:UIControlStateNormal];
