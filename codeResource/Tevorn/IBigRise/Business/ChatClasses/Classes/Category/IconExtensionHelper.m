...
@implementation NSString (TimeLag)

//: static char NIMKitStringJsonDictionaryAddress;
static char k_mutualName;
...
- (NSDictionary *)condition
{
    ...
    NSDictionary *dict = [objc_getAssociatedObject(self, &k_mutualName) copy];
    //: if (dict == nil) 
    if (dict == nil) //解析过一次后就缓存解析结果，避免多次解析
    {
        ...
        objc_setAssociatedObject(self,&k_mutualName,dict,OBJC_ASSOCIATION_COPY);
    }
    //: return dict;
    return dict;
...
@implementation NSDictionary (TimeLag)
